...
exports.mapPostsToTagsCategories = entities => {
  ...
  return entities.map(e => {
    ...
    if (tags.length && entityHasTags) {
      e.tags___NODE = e.tags.map(t => {
        const tagNode = tags.find(tObj => (Number.isInteger(t) ? t : t.wordpress_id) === tObj.wordpress_id);
    ...
    if (categories.length && entityHasCategories) {
      e.categories___NODE = e.categories.map(c => {
        const categoryNode = categories.find(cObj => (Number.isInteger(c) ? c : c.wordpress_id) === cObj.wordpress_id);
...
exports.mapPolylangTranslations = entities => entities.map(entity => {
  if (entity.polylang_translations) {
    entity.polylang_translations___NODE = entity.polylang_translations.map(translation => {
      const post = entities.find(t => t.wordpress_id === translation.wordpress_id && entity.__type === t.__type);
